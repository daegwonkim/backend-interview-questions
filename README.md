...
<details>
  ...
  동기와 비동기는 <b>프로그래밍 실행 방식</b>에 관한 개념으로, <b>작업의 완료를 기다리는지, 기다리지 않는지</b>에 따라 구분됩니다.
...
<details>
  ...
  블로킹(Blocking) I/O와 논블로킹(Non-blocking) I/O는 <b>I/O 작업 요청 시 호출한 함수가 즉시 반환되는지 여부</b>에 따라 구분됩니다.

  <b>블로킹(Blocking) I/O</b>는 I/O 작업을 요청했을 때 해당 작업이 완료될 때까지 호출한 함수가 반환되지 않고 대기하는 방식입니다. 이 과정에서 프로세스나 스레드는 다른 작업을 수행할 수 없으며, 커널이 I/O 작업을 완료하면 그 결과를 받아 다음 작업을 진행합니다.
  
  <b>논블로킹(Non-blocking) I/O</b>는 I/O 작업을 요청했을 때 작업의 완료 여부와 관계없이 즉시 반환되는 방식입니다. 커널은 데이터가 준비되지 않았더라도 바로 응답하며(예: `EAGAIN` 에러), 이를 통해 애플리케이션은 대기하지 않고 다른 작업을 수행할 수 있습니다. 이후 주기적으로 폴링(polling)이나 이벤트 통지를 통해 I/O 작업 완료를 확인합니다.
</details>

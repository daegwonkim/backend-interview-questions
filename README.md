...
<details>
  ...
  즉, 정해진 카운트에 따라 임계 영역에 동시에 접근할 수 있는 스레드의 개수가 정해지며, 해당 카운트를 감소시키고(wait) 증가시키는(signal) 연산으로 동기화가 이루어집니다.<br>
  ...
  따라서 모니터는 단순한 락 획득/해제 뿐만 아니라 조건 변수를 통한 조건 기반 대기와 통지 매커니즘을 제공하여 더 정교한 제어가 가능합니다. 또한 세마포어는 개발자가 직접 락을 관리해야 하지만, 모니터는 언어 차원에서 자동으로 처리되어 더 안전합니다.
</details>
...
<details>
  <summary>TCP의 오류제어, 흐름제어, 혼잡제어에 대해 설명해주세요.</summary><br>
  
  TCP의 **오류제어 기능**은 **패킷이 손실되었을 때 이를 재전송하여 모든 데이터가 목적지까지 정확하게 도착하도록 보장하는 기능**입니다. 송신 호스트에서 패킷이 손실되었음을 판단하는 기준은 다음과 같습니다.
  - 수신 호스트로부터 중복된 ACK 세그먼트를 수신 받았을 때
  - 타임아웃이 발생할 때까지 ACK 세그먼트를 수신 받지 못했을 때

  TCP에서는 오류 제어를 위해 **ARQ(Automatic Repeat reQuest)** 기법을 사용하며 여기에는 대표적인 세 가지 방식이 있습니다.
  - **Stop-and-Wait ARQ**: 제대로 전달했음을 확인하기 전까지는 새로운 메시지를 보내지 않는 방식으로, 단순하지만 높은 신뢰성을 보장할 수 있다는 장점이 있습니다. 단, 네트워크의 이용 효율이 낮아지고 성능이 저하된다는 단점이 있습니다. 이러한 단점을 해결하기 위해서는 각 세그먼트에 대한 ACK 세그먼트가 도착하기 전이라도 여러 세그먼트를 보낼 수 있어야 하며, 이러한 기술을 파이프라이닝이라고 합니다.
  - **Go-Back-N ARQ**: Stop-and-Wait ARQ의 단점을 보완하기 위한 파이프라이닝 기반 ARQ의 일종으로, 여러 세그먼트 전송 중 오류가 발생하면 해당 세그먼트부터 전부 재전송하는 방식입니다.
  - **Selective Repeat ARQ**: Go-Back-N ARQ의 ACK 세그먼트가 누적 확인 응답이었다면, Selective Repeat ARQ의 ACK 세그먼트는 개별 확인 응답으로 동작합니다. 즉, 전송에 실패한 패킷들에 대해서만 ACK 세그먼트를 재전송하는 방식입니다.

  TCP의 **흐름제어 기능**은 파이프라이닝 기반의 ARQ(Go-Back-N ARQ, Selective Repeat ARQ)에서 수신 호스트가 한 번에 얼마나 많은 세그먼트를 처리할 수 있는지를 확인하고 **수신 호스트의 처리 속도를 고려하여 송수신 속도를 균일하게 유지하기 위한 기능**을 말합니다. TCP에서는 흐름 제어를 위해 **슬라이딩 윈도우** 기법을 사용하여 정해진 윈도우 크기만큼의 세그먼트들을 한 번에 전송하는 방식으로 동작합니다.

  TCP의 **혼잡제어 기능**은 **송신 호스트가 혼잡한 정도에 맞춰 유동적으로 전송량을 조절하는 기능**을 말하며, 이때 혼잡 없이 전송할 수 있을 법한 데이터의 양을 혼잡 윈도우라고 부릅니다. 즉, 혼잡 위도우가 크다면 한 번에 전송할 수 있는 세그먼트 수가 많다는 것을 의미하며, 혼잡 윈도우가 작다면 한 번에 전송할 수 있는 세그먼트 수가 적다는 것(네트워크가 혼잡한 상황이라는 것)을 의미합니다. 흐름 제어의 주체가 수신 호스트였다면 혼잡 제어의 주체는 송신 호스트가 됩니다.
</details>
...
<details>
  ...
  <b>HTTP</b>는 웹에서 클라이언트와 서버간에 데이터를 주고받기 위한 요청-응답 기반 프로토콜로, 상태를 유지하지 않는 Stateless 프로토콜이며, TCP/IP 위에서 동작합니다.<br>
...
<details>
  ...
  <b>이진 탐색 트리(Binary Search Tree)</b>는 이진 트리 기반의 자료구조로, 탐색을 빠르게 하기 위해 <b>특별한 규칙</b>이 추가된 자료구조입니다.<br>

  이진 탐색 트리에서 <b>각 노드의 왼쪽 서브트리는 현재 노드보다 작은 값, 오른쪽 서브트리는 현재 노드보다 큰 값</b>을 갖습니다.

  이러한 규칙 덕분에 탐색을 진행할 때마다 탐색의 범위가 절반으로 줄어들어, 탐색에 평균적으로 O(log n)의 시간이 소요됩니다. 다만, 노드가 한쪽으로 치우친 형태이면 최악의 경우 O(n)까지 늘어날 수 있습니다.

  따라서 이를 보완하기 위해 스스로 트리의 균형을 유지하는 AVL 트리, Red-Black 트리 등을 주로 사용합니다.
</details>

<details>
  <summary>자가 균형 이진 탐색 트리에 대해 설명해주세요.</summary><br>

  자가 균형 이진 탐색 트리란 이진 탐색 트리에서 노드가 한쪽으로 치우쳤을 때 시간 복잡도가 늘어나는 문제를 방지하기 위해서 **스스로 트리의 균형을 유지하는 이진 탐색 트리**입니다.

  <b>AVL 트리</b>는 왼쪽, 오른쪽 서브트리의 높이 차이를 최대 1로 유지시키며, 높이 차이가 1보다 커지는 시점에 회전(Rotation)을 통해 트리의 균형을 잡아 높이 차이를 줄이는 자가 균형 이진 탐색 트리입니다.
  거의 완벽한 균형을 유지하여 트리의 높이를 항상 최소 수준으로 유지하기 때문에 탐색, 삽입, 삭제 모두 O(log N)을 보장합니다.

  <b>RB(Red-Black) 트리</b>는 각 노드에 색깔(빨간색 또는 검은색)을 부여하고, 특별한 색 규칙을 이용해 트리의 균형을 유지하는 자가 균형 이진 탐색 트리입니다.
  색 규칙이 깨지는 시점에 회전을 통해 균형을 맞추며, AVL 트리보다는 비교적 느슨한 균형을 유지하기 때문에 회전 발생 빈도가 적습니다.
</details>
...
<details>
  ...
  `StringBuffer`와 `StringBuilder`는 둘 다 <b>가변(mutable) 객체</b>입니다. 내부 버퍼를 가지고 있어서 문자열을 수정할 때 새로운 객체를 생성하지 않고 기존 버퍼의 내용을 변경합니다. 이 둘의 핵심적인 차이점은 <b>동시성 제어 여부</b>입니다.<br>
  - `StringBuffer`는 <b>thread-safe</b>합니다. 모든 메서드가 `synchronized`로 선언되어 있어서 멀티스레드 환경에서 안전하게 사용할 수 있습니다. 하지만 동시성 제어의 오버헤드로 인해 성능이 상대적으로 낮습니다.<br>
  - `StringBuilder`는 <b>thread-unsafe</b>합니다. 동시성 제어가 되어 있지 않아서 멀티스레드 환경에서 안정성이 낮지만, 그만큼 성능은 높습니다.
...
<details>
  ...
  스프링 애플리케이션이 실행되면 `ApplicationContext`라는 스프링 컨테이너가 생성되고, 스프링 빈으로 정의된 객체를 스캔합니다.

  이후 의존성 그래프를 분석하여 스프링 빈 생성 순서를 결정하고, 의존성 주입이 완료된 스프링 빈은 스프링 컨테이너에 등록되며, 스프링 컨테이너가 애플리케이션이 종료될 때까지 이를 관리하게 됩니다.
</details>
...
<details>
  <summary>낙관적 락과 비관적 락의 차이에 대해 설명해주세요.</summary><br>

  비관적 락은 데이터 충돌이 발생할 가능성이 높다고 보고, 트랜잭션 시작 시점에 DB 락을 걸어 다른 접근을 막는 방법입니다.

  반면 낙관적 락은 충돌이 드물다고 가정하고 락 없이 처리한 뒤, 커밋 시점에 버전 비교를 통해 충돌을 감지합니다.

  일반적으로 낙관적 락이 DB 락으로 인한 대기가 없어서 동시성이 높기 때문에 성능이 더 좋지만, 충돌 시 재시도해야 하는 비용이 있기 때문에 충돌이 잦으면 오히려 성능이 떨어질 수 있습니다.
</details>
